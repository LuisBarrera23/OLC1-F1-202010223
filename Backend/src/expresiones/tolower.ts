...
export class Tolower extends Expression{
    ...
    public ejecutar(env: Environment): Retorno {
        ...
        if(exp.type!=Type.STRING){
            throw instancia.addError(new Error("Semantico","Tolower necesita una expresion de tipo String",this.line,this.column+1));
        }
        let cadena:string=exp.value;
        resultado={value:cadena.toLowerCase(),type:Type.STRING}
        return resultado;
    }
}